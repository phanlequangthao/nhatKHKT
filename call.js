...
async function sendFramesToServer() {
    ...
    try {
        ...
        if (result.label && result.confidence) {
            updateLabelDisplay(result.label, result.confidence);
            
            if (result.label === lastLabel) {
                labelCounter++;
                if (labelCounter >= LABEL_THRESHOLD) {
                    if (result.label !== lastResult) {
                        appendResult(result.label);
        ...
        console.error('Failed to send frames to server:', error);
    } finally {
        // Clear the frame buffer regardless of success or failure
        frameBuffer = [];
    }
}

// Assume this function is called when the camera is ready
function startFrameCapture() {
    setInterval(captureAndSendFrame, 100); // Capture a frame every 100ms
}
...
function appendResult(newResult) {
    ...
    if (newResult !== lastResult) {
        chatInput.value = currentText ? `${currentText}${newResult}` : newResult;
